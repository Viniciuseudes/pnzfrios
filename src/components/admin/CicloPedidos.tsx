...
import {
  Search, Layers, Wallet, Zap, Circle, ChevronRight,
  Eye, Clock,
} from "lucide-react";
import { KANBAN_COLS } from "@/data/kanban";
...
function KanbanCard({ order, onClick, onQuickAdvance, isAdmin }: {
  order: KanbanOrder; onClick: () => void;
  onQuickAdvance: (id: string) => void; isAdmin: boolean;
}) {
  ...
  const stuck = age > 8 && order.status !== "entregue" && order.status !== "cancelado";
  const sellerColors: Record<string, string> = { VE: "bg-amber-600", LF: "bg-[#1e4023]", TG: "bg-[#2d6a3a]", VJ: "bg-[#c8921c]" };
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      ...
      onClick={onClick}>
      <div className={`h-1 rounded-t-xl ${order.priority === "Urgente" ? "bg-red-500" : col.dot}`} />
      <div className="p-3">
        <div className="flex items-start justify-between gap-1 mb-2">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[10px] font-black text-muted-foreground">{order.orderNumber}</span>
            ...
            {stuck && (
              <span className="flex items-center gap-0.5 text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-600 border border-amber-100">
                <Eye className="w-2 h-2" />{Math.floor(age)}h parado
              </span>
            )}
          </div>
          <span className="text-[10px] text-muted-foreground/50 flex-shrink-0">{timeAgo(order.createdAt)}</span>
        </div>
        <p className="text-xs font-bold text-foreground leading-tight mb-1 line-clamp-1">{order.clientName}</p>
        <div className="flex items-center gap-1.5 mb-2.5">
          <div className={`w-4 h-4 rounded-full ${sellerColors[order.sellerAvatar] ?? "bg-gray-400"} flex items-center justify-center text-[8px] font-bold text-white flex-shrink-0`}>
            {order.sellerAvatar[0]}
          </div>
          <span className="text-[10px] text-muted-foreground truncate">{order.sellerName.split(" ")[0]}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-[10px] text-muted-foreground">{order.items.length} {order.items.length === 1 ? "item" : "itens"} · {order.items.reduce((a, i) => a + i.qty, 0)} kg</span>
          <span className="text-xs font-black text-[#1e4023]">{fmt(order.total)}</span>
        </div>
        <p className="text-[10px] text-muted-foreground/60 mt-1 truncate flex items-center gap-1">
          <MapPin className="w-2.5 h-2.5 flex-shrink-0" />{order.deliveryAddress.split("—")[1]?.trim() ?? order.deliveryAddress}
        </p>
        {isAdmin && nextCol && (
          <button onClick={e => { e.stopPropagation(); onQuickAdvance(order.id); }}
...
export function CicloPedidos({ orders, onAdvance, onCancel, onAddNote }: {
  orders: KanbanOrder[];
  ...
  const [priorityFilter, setPriorityFilter] = useState<"todas" | "Urgente">("todas");
  ...
  const filtered = orders.filter(o => {
    ...
    if (search && !o.clientName.toLowerCase().includes(search.toLowerCase()) && !o.orderNumber.includes(search)) return false;
    return true;
  });

  const activeCols = KANBAN_COLS.filter(c => c.id !== "cancelado");
  const totalPipeline = orders.filter(o => !["entregue", "cancelado"].includes(o.status)).reduce((a, o) => a + o.total, 0);
  const urgentCount = orders.filter(o => o.priority === "Urgente" && !["entregue", "cancelado"].includes(o.status)).length;
      ...
      <div className="space-y-5">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Do pedido ao cliente — rastreamento em tempo real</p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-2 bg-card border border-border rounded-xl px-3 py-1.5 text-xs">
              ...
              <span className="font-black text-[#1e4023]">{fmt(totalPipeline)}</span>
            </div>
            {urgentCount > 0 && (
              <div className="flex items-center gap-1.5 bg-red-50 border border-red-200 rounded-xl px-3 py-1.5 text-xs text-red-600 font-semibold">
                <Zap className="w-3 h-3" />{urgentCount} urgente(s)
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select value={sellerFilter} onChange={e => setSellerFilter(e.target.value === "todos" ? "todos" : Number(e.target.value))}
            className="px-3 py-2 rounded-lg border border-border bg-card text-xs focus:outline-none focus:ring-2 focus:ring-primary/30 transition">
            <option value="todos">Todos vendedores</option>
            {sellers.map(s => <option key={s.id} value={s.id}>{s.name.split(" ")[0]}</option>)}
          </select>
          <button onClick={() => setPriorityFilter(p => p === "todas" ? "Urgente" : "todas")}
            ...
            <Zap className="w-3 h-3" /> {priorityFilter === "Urgente" ? "Somente urgentes" : "Filtrar urgentes"}
          </button>
        ...
        <div className="overflow-x-auto pb-4 -mx-4 px-4">
          <div className="flex gap-3" style={{ minWidth: `${activeCols.length * 272}px` }}>
            {activeCols.map(col => {
              ...
              return (
                <div key={col.id} className="w-[264px] flex-shrink-0 flex flex-col">
                  <div className={`rounded-xl border ${col.border} ${col.bg} px-3 py-2.5 mb-2 flex items-center justify-between flex-shrink-0`}>
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${col.dot}`} />
                      <span className={`text-xs font-bold ${col.color}`}>{col.label}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-full ${col.dot} text-white`}>{colOrders.length}</span>
                      {colOrders.length > 0 && <span className={`text-[9px] font-semibold ${col.color} opacity-70`}>{fmt(colTotal)}</span>}
                    </div>
                  </div>
                  <div className="flex-1 space-y-2 overflow-y-auto pr-0.5" style={{ maxHeight: "calc(100vh - 320px)", minHeight: 80 }}>
                    <AnimatePresence>
                      {colOrders.length === 0 ? (
                        ...
                        <KanbanCard key={order.id} order={order} isAdmin={true}
        ...
        {showCancelled && cancelledOrders.length > 0 && (
          <div>
            <p className="text-xs font-semibold text-red-500 uppercase tracking-wide mb-2 flex items-center gap-1.5">
              <X className="w-3.5 h-3.5" /> Pedidos Cancelados ({cancelledOrders.length})
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
              {cancelledOrders.map(o => (
                <KanbanCard key={o.id} order={o} isAdmin={true}
                  onClick={() => setSelected(o)} onQuickAdvance={() => {}} />
              ))}
            </div>
          </div>
        )}
