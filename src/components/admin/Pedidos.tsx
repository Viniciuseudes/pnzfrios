...
import { Search, X, Trash2, CheckCircle, Package, User2 } from "lucide-react";
...
export function Pedidos() {
  ...
  async function handleConfirmOrder() {
    ...
    try {
      // LÓGICA SÊNIOR: Pega o número do último pedido e soma +1
      ...
      if (lastOrder && lastOrder.length > 0 && lastOrder[0].order_number) {
        const numStr = lastOrder[0].order_number.replace(/\D/g, ""); // Extrai só os números
      ...
      const { data: order, error: orderError } = await supabase
        ...
        .insert({
          ...
          seller_id: selectedSellerId, // Agora usa o vendedor selecionado
          priority: "Normal",
          status: "novo",
      ...
      await supabase.from("order_history").insert({
        ...
        note: "Pedido criado via painel interno",
      });
      ...
      {loadingData ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            ...
            {/* Step 2: Produtos (Permanece igual ao seu original) */}
            <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-6 h-6 rounded-full bg-primary text-primary-foreground text-xs font-bold flex items-center justify-center">
                  2
            ...
            <div className="bg-card rounded-xl border border-border p-5 shadow-sm sticky top-4">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <span>
                    {orderItems.reduce((a, i) => a + i.qty, 0)} unidades
                  </span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>Subtotal</span>
              ...
              {(!selectedClient ||
                ...
                <p className="text-xs text-muted-foreground text-center mt-2">
                  Preencha cliente, vendedor e adicione produtos.
                </p>
              )}
            </div>
          </div>
