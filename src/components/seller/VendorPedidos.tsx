  ...
  async function fetchOrders() {
    ...
    const [{ data: sData }, { data: oData }] = await Promise.all([
      ...
      supabase
        .from("orders")
        .select(
          `id, order_number, status, priority, created_at, updated_at, notes, delivery_address, client_id, clients(name), sellers(name, avatar), order_items(product_id, qty, price, products(name)), order_history(status, changed_by, note, created_at)`,
        )
